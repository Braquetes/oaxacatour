...
import com.example.oaxacatour.Lugares;
...
public class SantoDomingoActivity extends AppCompatActivity {
   ...
   private void data(){
       mDatabase.child("santoDomingo").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
           @Override
           public void onComplete(@NonNull Task<DataSnapshot> task) {
               if (task.isSuccessful()) {
                   ...
                   Glide.with(SantoDomingoActivity.this).load(ar.get(3)).into(imagen);

               }
               else {
                   Toast.makeText(SantoDomingoActivity.this, "Error", Toast.LENGTH_SHORT).show();
